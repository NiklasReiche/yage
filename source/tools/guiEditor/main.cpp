...
public:
	...
	void on_radio_click()
	{
		label_radio->setText("state: " + util::to_string(radioGroup->getState()));
	}
...
void displayFPS(GuiTest& gui, double dt)
{
	...
	gui.fpsCounter->setText(util::to_string((int)frameRate));
}

int main()
{
	sys::PlatformHandle platformHandle;
	gl::GraphicsContext glContext(&platformHandle, 1400, 800);
