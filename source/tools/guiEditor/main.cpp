...
public:
	...
	GuiTest(sys::Window platform, gl::Context gl)
		: platform(platform), gl(gl)
	{
		master = new gui::Master(platform, gl);
		try {
			master->addFont(installPath + "/res/arial.font");
		}
		catch (sys::FileException& exception) {
			//((sys::desktop::GlfwWindow)platform)->log("ERROR::FONT_LOADER: could not open file " + exception.file());
		}
	...
	{
		clicks++;
		label_clicks->setText("clicks: " + util::toString(clicks));
	...
	void on_radio_click()
	{
		label_radio->setText("state: " + util::toString(radioGroup->getState()));
	}
...
void displayFPS(GuiTest& gui, double dt)
{
	...
	gui.fpsCounter->setText(util::toString((int)frameRate));
}

int main()
{
	auto window = std::make_shared<sys::desktop::GlfwWindow>();
	auto glContext = std::static_pointer_cast<sys::desktop::GlfwWindow>(window)->createContext(1400, 900);
	
	GuiTest guiTest(window, glContext);
