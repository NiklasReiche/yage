...
namespace img::png
{
	...
	void user_error_fn(png_structp png_ptr, png_const_charp error_msg)
	{
		...
		throw sys::FileException(sys::FileException::BAD_IO);
	}
	...
	void PNG::load(const std::string & filename, FORCE_CHANNELS forceChannel)
	{
		sys::File file = sys::FileLoader::openFile(filename);

		if (!isValid(file)) {
			throw sys::FileException(sys::FileException::BAD_FILE, "not valid png", filename);
		}

		png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, (png_voidp)this, user_error_fn, user_warning_fn);
		if (!png_ptr) {
			this->free();
			throw sys::FileException(sys::FileException::BAD_FILE, "file corrupted", filename);
		}
