...
namespace opengl
{
	...
	std::unique_ptr<gl::ITexture2D> TextureCreator::createTexture2D(
		int width,
		int height,
	    gl::ImageFormat textureFormat,
	    const std::span<unsigned char>& data,
	    gl::PixelTransferParams params)
	{
		...
		auto texture = std::unique_ptr<Texture2D>(new Texture2D(ptr));
	...
	std::unique_ptr<gl::ICubemap> TextureCreator::createCubemap(
		int width,
		int height,
		gl::ImageFormat textureFormat,
		...
		gl::PixelTransferParams params)
	{
		...
		auto ptr = lockContextPtr();
		auto texture = std::unique_ptr<Cubemap>(new Cubemap(ptr));
