...
namespace yage::platform::desktop
{
    GlfwWindow::GlfwWindow(const int width, const int height, const std::string& title, const gl::Api gl_api)
    {
...
#endif
                break;
            case gl::Api::API_VULKAN: glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API); break;
            default:                  throw std::invalid_argument("Unsupported gl-api");
        }

        glfwWindowHint(GLFW_VISIBLE, GLFW_FALSE);
        glfwWindowHint(GLFW_RESIZABLE, GL_TRUE);

        m_glfw_handle = glfwCreateWindow(width, height, title.c_str(), nullptr, nullptr);

        if (m_glfw_handle == nullptr) {
            glfwTerminate();
        ...
        glfwSetCharCallback(m_glfw_handle, onCharEvent);
        glfwSetKeyCallback(m_glfw_handle, onKeyEvent);
        glfwSetCursorPosCallback(m_glfw_handle, onMousePosEvent);
        glfwSetMouseButtonCallback(m_glfw_handle, onMouseButtonEvent);
        glfwSetScrollCallback(m_glfw_handle, onMouseWheelEvent);
        glfwSetFramebufferSizeCallback(m_glfw_handle, on_framebuffer_resize_event);
    ...
    {
        glfwSetInputMode(m_glfw_handle, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
    }

    void GlfwWindow::showCursor()
    {
        glfwSetInputMode(m_glfw_handle, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
    }

    int GlfwWindow::should_destroy()
    {
    ...
    {
        m_char_input_enabled = true;
    }

    void GlfwWindow::disableCharInput()
    {
        m_char_input_enabled = false;
    }

    void GlfwWindow::enableKeyInput()
    {
        m_key_input_enabled = true;
    }
    ...
    double GlfwWindow::getTimeStep()
    {
        ...
        const double step = now - m_last_time_step;
        m_last_time_step = now;
        return step;
    }

    void GlfwWindow::attach(input::InputListener& listener)
    {
        m_input_listeners.emplace_back(listener);
    }
    ...
    {
        for (auto listener: m_input_listeners) {
    ...
    void GlfwWindow::onCharEvent(GLFWwindow* window, const unsigned int codepoint)
    {
        ...
        if (!handle->m_char_input_enabled) {
            return;
        }
    ...
    std::string GlfwWindow::openFileDialog(std::string defaultPath, std::vector<std::string> filterList,
                                           std::string filterName)
    {
        ...
        const char* result = tinyfd_openFileDialog("Open File", defaultPathC, filterListC.size(), &filterListC[0],
                                                   filterName.c_str(), 0);
