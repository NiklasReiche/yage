...
{
#if 0
	gl::Shader GL3_ShaderCreator::loadFromFile(std::string vertex_path, std::string fragment_path, std::string geometry_path)
	{
...
#endif
	void GL3_ShaderCreator::checkShaderCompilationError(const GLuint program, const ShaderType type)
	{
		int success;
		char infoLog[512];
	...
	std::unique_ptr<gl::IShader> GL3_ShaderCreator::createShader(
		const std::string& vertexCode,
		const std::string& fragmentCode,
		const std::string& geometryCode)
	{
		auto shader = std::unique_ptr<GL3_Shader>(new GL3_Shader(lockContextPtr()));
